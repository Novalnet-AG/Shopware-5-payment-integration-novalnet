...
class PluginInstaller
{
    ...
    private function createNovalnetPayments()
    {
        ...
        $shopRepository = $this->modelManager->getRepository('Shopware\Models\Shop\Shop');
        ...
        foreach ($payments as $paymentName) {
            ...
            $existingPayment = $this->modelManager->getRepository('Shopware\Models\Payment\Payment')->findOneBy(['name' => $paymentName]);
    ...
    {
        return [
            $this->modelManager->getClassMetadata('Shopware\CustomModels\Transaction\Transaction'),
            $this->modelManager->getClassMetadata('Shopware\CustomModels\Api\Api')
        ];
    }
    ...
    public function removeNovalnetApiDetails()
    {
        ...
        if (!empty($count)) {
            ...
            $apiRepository = $this->modelManager->getRepository('Shopware\CustomModels\Api\Api');
    ...
    public function deactivatePayments()
    {
        $payments = array_keys($this->helper->getPaymentInfo());
        foreach ($payments as $paymentName) {
            $paymentType =  $this->modelManager->getRepository('Shopware\Models\Payment\Payment')->findOneBy([
                'name' => $paymentName,
            ]);
            if ($paymentType) {
