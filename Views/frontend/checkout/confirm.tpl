...
{block name="frontend_checkout_confirm_left_payment_method"}
    ...
    {/if}
    {if $maskedDetails.tid neq '' && $sUserData.additional.payment.name eq 'novalnetpaypal'}
        <div id="novalnetpaypal_ref_details" >
            <p class="none">
				<label  style="width:50%;"><strong>{s namespace='frontend/novalnet/payment' name='frontend_novalnet_paypal_reference_tid'}Novalnet transaction ID{/s}: </strong>{$novalnetpaypal_account_details.tid}</label>
			</p>
		</div>
    {/if}
{/block}
	...
	{if $sPayment.name == 'novalnetapplepay'}
		<div class="table--actions actions--bottom">
			<div class="main--actions">
				{include file='frontend/noval_payment/button.tpl' pageType = 'checkout'}
			</div>
		</div>
	{else}
		{$smarty.block.parent}
