...
{block name='frontend_checkout_finish_payment_method'}
	{if $sPayment.name eq 'novalnetpay' && $maskedDetails.result.status eq 'SUCCESS' && $maskedDetails.payment_details.name ne ''}
		<strong>{s name="ConfirmHeaderPayment" namespace="frontend/checkout/confirm"}{/s}:</strong> {$maskedDetails.payment_details.name}<br />
	{elseif $nnOrderPaymentName != '' || $nnOrderPaymentName}
		<strong>{s name="ConfirmHeaderPayment" namespace="frontend/checkout/confirm"}{/s}:</strong> {$nnOrderPaymentName}<br />
	{else}
		{$smarty.block.parent}
	{/if}
{/block}
