...
{if (!isset($sArticle.active) || $sArticle.active)}
	{if $sArticle.isAvailable && (!$sArticle.sConfigurator || $activeConfiguratorSelection) }
		...
		{foreach from=$nnWalletPayments key=k item=nnPaymentName}
			{if is_array($nnConfig["{$nnPaymentName}_button_display_fields"]) && 'index'|in_array:$nnConfig["{$nnPaymentName}_button_display_fields"]}
