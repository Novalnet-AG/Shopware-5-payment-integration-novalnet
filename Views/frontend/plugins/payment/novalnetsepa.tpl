	...
	<div id="{$payment_mean.name}PaymentForm" class="{$payment_mean.name}PaymentForm">
		...
		{if $nnConfig["{$payment_mean.name}_shopping_type"] eq '1' && $sUserData.additional.user.accountmode eq '0' }
			<div class="saveCreditCardData">
				...
				<label for="saveSepaCardData">{s namespace='frontend/novalnet/payment' name='frontend_novalnetsepa_save_card'} Speichern für zukünftige Einkäufe {/s}</label>
			</div>
		{/if}
	</div>
	{if $payment_mean.name eq 'novalnetsepaGuarantee' || $payment_mean.name eq 'novalnetsepainstalment' }
		<div class="{$payment_mean.name}DobField" id="{$payment_mean.name}DobField" style="display:{$display}">
			<p class="nnclass">
