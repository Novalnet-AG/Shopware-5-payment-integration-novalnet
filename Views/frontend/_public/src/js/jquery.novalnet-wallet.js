...
(function($) {
    ...
    $.plugin('novalnetWalletPay', {
         ...
         */
        _loadWalletPaymentForm: function () {
			var me = this,
			...
			{
				configuration.transaction.amount = parseInt($('#nnArticlePrice').val());
			}
			...
			novalnetPaymentObj.isPaymentMethodAvailable(function(displayPaymentMethod) {
				...
				if (displayPaymentMethod) {
					...
					// set widht for apple pay
					if (me.$el.find('apple-pay-button') != null && me.$el.find('apple-pay-button') != undefined && me.$el.find('apple-pay-button').length > 0)
					{
						me.$el.find('apple-pay-button').css("width", "100%");
					}
				}
			});
		},
