...
(function ($) {
    ...
    $.plugin('novalnetPayments', {
        ...
        loadPaymentForm : function () {
            ...
            var me                  = this,
                ...
                orderInfoData       = JSON.parse(walletPaymentParams),
            ...
            paymentFormInstance.selectedPayment((data) => {
                ...
                if ($('#shippingPaymentForm').length) {
                    ...
                    $('#payment_mean' + novalnetId.val()).addClass('radio auto_submit');
                    $('#payment_mean' + novalnetId.val()).attr('checked','checked');
                }

                $('#payment_mean' + novalnetId.val()).trigger('click');
                
                if ($('input[name="payment"]:checked').val() == $('#novalnetpayId').val() && $('#novalnetpayId').val() != $('#formDataId').val()) {
    ...
    if (typeof jQuery != 'undefined') {
        window.StateManager.addPlugin('.novalnet-payments', 'novalnetPayments');
         ...
         $.subscribe('plugin/swAboPaymentSelection/onRegisterPlugins', function () {
            window.StateManager.addPlugin('.abo-commerce-payment--selection-form', 'novalnetPayments');
         });
    }
