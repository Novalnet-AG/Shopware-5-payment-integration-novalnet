...
class Shopware_Controllers_Frontend_NovalPayment extends Shopware_Controllers_Frontend_Payment implements CSRFWhitelistAware
{
    ...
    public function gatewayAction()
    {
        ...
        if ((in_array($this->paymentShortName, $this->nnRedirectPayments) && $this->paymentShortName != 'novalnetpaypal') || ($this->paymentShortName == 'novalnetcc' && !empty($this->session->novalnet['novalnetcc']['nn_cc_new_acc_details']) && !empty($this->session->novalnet['novalnetcc']['nn_cc_do_redirect'])) || ($this->paymentShortName == 'novalnetpaypal' && !empty($this->session->novalnet['novalnetpaypal']['nn_paypal_new_acc_details']))) {
            $redirectFlag                           = $this->paymentShortName . '_gatewayAction';
            $this->session->$redirectFlag	= 1;
            if(empty($_SESSION['novalnet']['isOrderCreated']))
            {
                $orderNumber                            = $this->saveOrder($this->uniquePaymentID, $this->uniquePaymentID);
                $_SESSION['novalnet']['novalnetOrderNo']= $orderNumber;
                $_SESSION['novalnet']['isOrderCreated'] = 1;
            }
            $this->param['input3']                  = 'payment_temporary_id';
            $this->param['inputval3']               = $this->uniquePaymentID;
            $this->param['order_no']                = ($orderNumber) ? $orderNumber : $_SESSION['novalnet']['novalnetOrderNo'];
        }
    ...
    {   
        ...
        if ((in_array($this->paymentShortName, $this->nnRedirectPayments) && $this->paymentShortName != 'novalnetpaypal') || ($this->paymentShortName == 'novalnetcc' && $this->configDetails['novalnetcc_force_cc3d']) || ($this->paymentShortName == 'novalnetpaypal' && !empty($this->session->novalnet['novalnetpaypal']['nn_paypal_new_acc_details']))) {
            
            $this->session->context = array_merge($this->session->context, array('sComment' => $this->session->sComment, 'ordernumber' => $novalnetResult['order_no'], 'sOrderNumber' => $novalnetResult['order_no']));
            $this->session->context = array_merge($this->session->context, array('sNet' => empty($this->user['additional']['charge_vat'])));
        ...
        if (!empty($orderNumber)) {
            ...
            if (!in_array($this->paymentShortName, $this->nnRedirectPayments) || ($this->paymentShortName == 'novalnetcc' && !$this->configDetails['novalnetcc_force_cc3d'])) {
