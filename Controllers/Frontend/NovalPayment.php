...
class Shopware_Controllers_Frontend_NovalPayment extends Shopware_Controllers_Frontend_Payment implements CSRFWhitelistAware
{
    ...
    {
        return ['return', 'cancel', 'status', 'recurring', 'deleteCard', 'addArticle', 'getAvailableShipping', 'createApplePayOrder'];
    }
    ...
    public function processRequestAction()
    {
        ...
        foreach ($basket['content'] as $content) {
            if (!empty($content['abo_attributes'])) {
                ...
                if (!empty($params['transaction']['payment_data']) && empty($params['transaction']['payment_data']['token'])) {
        ...
        $isRedirect = (in_array($this->paymentShortName, $this->helper->getRedirectPayments())) || (in_array($this->paymentShortName, $this->helper->getEnforcePayments()) && ($sessionData['doRedirect'] == 1 || $sessionData['doRedirect'] == 'true'));
    ...
    public function cancelAction()
    {
        ...
        if (empty($transactionDetails['tid'])) {
            return $this->redirect($this->errorUrl . '?sNNError=' . $novalnetResponse['status_text']);
        }
    ...
    public function recurringAction()
    {
        ...
        $data = $this->helper->removeEmptyArrayElements($data);
        
        if (!empty($this->paymentShortName) && in_array($this->paymentShortName, array('novalnetcc', 'novalnetsepa', 'novalnetsepaGuarantee', 'novalnetpaypal')) && empty($data['transaction']['payment_data']['token'])) {
            $paymentRefDetails  = Shopware()->Db()->fetchRow('SELECT order_id, last_order_id FROM s_plugin_swag_abo_commerce_orders WHERE last_order_id = ?', [$this->Request()->getParam('orderId')]);
            $parentOrder = Shopware()->Modules()->Order()->getOrderById($paymentRefDetails['order_id']);
            $paymentRefData = $parentOrder['transactionID'];

            if (empty($paymentRefData)) {
                $orderNumber = !empty($parentOrder['ordernumber']) ? $parentOrder['ordernumber'] : $parentOrder['order_number'];
                $paymentRefData  = Shopware()->Db()->fetchOne('SELECT tid FROM s_novalnet_transaction_detail WHERE order_no = ? AND tid IS NOT NULL', [$orderNumber]);
            }

            $data['transaction']['payment_data']['payment_ref'] = $paymentRefData;
        }
        
        if (in_array($this->paymentShortName, array('novalnetsepaGuarantee', 'novalnetinvoiceGuarantee')) && empty($data['customer']['billing']['company'])) {
    ...
    public function handleResponse($response, $userData)
    {
        ...
        if ($response['result']['status'] == 'SUCCESS') {
            ...
            if ($response['transaction']['tid'] && $orderNumber && !in_array($this->paymentShortName, ['novalnetinvoiceGuarantee', 'novalnetinvoice', 'novalnetprepayment', 'novalnetinvoiceinstalment'])) {
    ...
    public function createApplePayOrderAction()
    {
        ...
        $requiredFields = array('firstName', 'lastName', 'addressLines', 'postalCode', 'locality', 'countryCode');
        foreach ($requiredFields as $key) {
            if (empty($response['order']['billing']['contact'][$key])) {
                $this->jsonResponse([
                    'success' => false,
                    'error'   => $this->helper->getLanguageFromSnippet('billingAddressError')
                ]);
                return;
            } elseif (empty($response['order']['shipping']['contact'][$key])) {
                $this->jsonResponse([
                    'success' => false,
        ...
        if (empty($userData['billingaddress'])) {
            $customer = $this->helper->createNewCustomer($response);
            $this->session->offsetSet('sUserId', $customer->getId());
            $userData = $this->helper->getUserInfo();
        } elseif (!empty($response['order']['billing']) && !empty($response['order']['shipping'])) {
            $userData = $this->helper->updateCustomerData($userData, $response);
        }

        $payment = Shopware()->Modules()->Admin()->sGetPaymentMeanById($this->session['sPaymentID'], $this->View()->$userData);
        if (!empty($postData['paymentName'])) {
            $paymentID  = Shopware()->Db()->fetchOne('SELECT id FROM s_core_paymentmeans WHERE name = ?', array($postData['paymentName']));
        ...
        $this->session->offsetSet('novalnet', new ArrayObject(['walletToken' => $response['transaction']['token'], 'doRedirect' => $response['transaction']['doRedirect']], ArrayObject::ARRAY_AS_PROPS));
        ...
        $serverResponse = $this->requestHandler->curlRequest($params, $endpoint);
    ...
    public function addArticleAction()
    {
        ...
        if (!empty($productId)) {
            ...
            $this->jsonResponse([
                'success' => false,
                'url' => $this->errorUrl . '?sNNError= Product Id is not found'
            ]);
            return;
        }
    }
