...
class Shopware_Controllers_Backend_NovalPayment extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function showTabForOurPaymentsAction()
    {
        ...
        if (preg_match("/INSTALMENT/i", $this->nnPaymentType()) && ! empty($this->getNNConfigDetails()['InstalmentDetails'])) {
            $instalmentCancelExecuted = ( empty($this->getNNConfigDetails()['instalmentCancelExecuted']) ) ? false : $this->getNNConfigDetails()['instalmentCancelExecuted'] ;
        ...
        if ($this->nnGatewayStatus()  == NOVALNET_CONFIRMED_STATUS && $this->reference->getRefundedAmount() < $this->reference->getAmount() && ! preg_match("/MULTIBANCO/i", $this->nnPaymentType())) {
            return $this->View()->assign(['success' => true]);
        }
        ...
        if (!empty($this->reference)) {
            if (! (preg_match("/INSTALMENT/i", $this->nnPaymentType()) ||  preg_match("/MULTIBANCO/i", $this->nnPaymentType()))) {
                ...
                return $this->View()->assign([
                    'success' => ( $this->nnGatewayStatus()  == NOVALNET_CONFIRMED_STATUS  &&
                                   $this->reference->getRefundedAmount() < $this->reference->getAmount() &&
                                   $balanceAmount > 0
                                 ),
    ...
    public function processBookingAmountAction()
    {
        ...
        $merchantData = !empty($this->getNNConfigDetails()['merchant']) ? $this->getNNConfigDetails()['merchant'] : $services->getMerchantDetails();
        ...
        $data = [
            ...
            'transaction'=> [
                ...
                'test_mode'        => $this->getNNConfigDetails()['test_mode'],
        ...
        if ($this->getNNConfigDetails()['token']) {
            $data['transaction']['payment_data']['token'] = $this->getNNConfigDetails()['token'];
        }
        ...
        if ($bookingResponse['result']['status'] === 'SUCCESS') {
                ...
                $bookingResponse['transaction']['payment_name'] = $this->getNNConfigDetails()['payment_name'];
                $message = $this->newLine . $this->helper->formCustomerComments(array_filter($bookingResponse), $bookingResponse['transaction']['currency']);
                ...
                $message  .= $this->newLine . $this->helper->getLanguageFromSnippet('frontend_novalnet_booking_process_message', $request['number']);
    ...
    {
        if (! empty($this->reference)) {
            return $this->View()->assign([ 'success' => ( ! empty(preg_match("/INSTALMENT/i", $this->nnPaymentType())) && $this->nnGatewayStatus() != NOVALNET_ON_HOLD_STATUS && ! empty($this->getNNConfigDetails()['InstalmentDetails']) ) ]);
        }
    ...
    public function displayPaymentInstalmentDetailsAction()
    {
        if (!empty($this->reference->getConfigurationDetails())) {
            ...
            foreach ($this->getNNConfigDetails()['InstalmentDetails'] as $key => $value) {
                if (is_array($value)) {
            ...
            $this->view->assign([
                'success' => true,
                'total' => count($this->getNNConfigDetails()['InstalmentDetails']),
                'data' => $instalmentInfo,
            ]);
        } else {
            $this->view->assign([
    ...
    public function displayInstalmentCancelAction()
    {
        if (!empty($this->reference)) {
            ...
            return $this->View()->assign([
                'success' => ( !empty(preg_match("/INSTALMENT/i", $this->nnPaymentType())) &&
    ...
    {
        $nnCofigDetails = $this->helper->unserializeData($this->reference->getConfigurationDetails());
        return $nnCofigDetails;
    }
    ...
    {
        return ! empty($this->reference->getPaymentType()) ? $this->helper->getPaymentType($this->reference->getPaymentType()) :null;
    }
    ...
    {
        return ! empty($this->reference->getGatewayStatus()) ? $this->helper->getStatus($this->reference->getGatewayStatus(), $this->reference, $this->nnPaymentType()) :null;
    }
    
    public function canShowCancelAllInstalments()
    {
        $cancelAllInstalment = null;
        $instalmentDetails = $this->getNNConfigDetails()['InstalmentDetails'];
    ...
    {
        $request = $this->Request()->getParams();
        if (!empty($this->getNNConfigDetails()['payment_name'])) {
            $novalnetPaymentName = $this->getNNConfigDetails()['payment_name'];
        } else {
    ...
    {
        $hasBankData = ! empty($this->getNNConfigDetails()) && $this->getNNConfigDetails() ['account_holder'] && $this->getNNConfigDetails() ['bank_name'] && $this->getNNConfigDetails() ['bank_place'] && $this->getNNConfigDetails() ['iban'] ? true : false;
        return $hasBankData;
    }
    ...
    {
        $shopId     = !empty($this->Request()->getParam('shopId')) ? $this->Request()->getParam('shopId') : $this->Request()->getParam('languageIso');
        $accessKey  = $this->databaseHandler->checkExistingApi($shopId)->getAccessKey();
