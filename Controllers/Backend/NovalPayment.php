...
class Shopware_Controllers_Backend_NovalPayment extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function processCaptureAction()
    {
        ...
        if ($captureResponse['result']['status'] === 'SUCCESS') {
            ...
            if ($this->reference->getPaymentType() == 'novalnetpaypal' && !empty($this->reference->getConfigurationDetails())) {
                $existingPaypalToken = $this->helper->unserializeData($this->reference->getConfigurationDetails());
                $paypalData['paypal_account']	= $captureResponse['transaction']['payment_data']['paypal_account'];
                $paypalData['paypal_transaction_id']	= $captureResponse['transaction']['payment_data']['paypal_transaction_id'];
                $paypalData['token'] = $existingPaypalToken['token'];

                $serializedData = $this->helper->serializeData($paypalData);
            }
