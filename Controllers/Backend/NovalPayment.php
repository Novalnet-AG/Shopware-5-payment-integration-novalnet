...
class Shopware_Controllers_Backend_NovalPayment extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function displayRefundTabAction()
    {
        if (!empty($this->reference)) {
            if ( ! in_array ( $this->reference->getPaymentType(), [ 'novalnetinvoiceinstalment', 'novalnetsepainstalment'] ) ) {
                ...
                return $this->View()->assign( ['success' => ( (( in_array( $this->reference->getGatewayStatus() , [ 'CONFIRMED' , 100 ] ) ) && ($this->reference->getRefundedAmount() < $this->reference->getAmount()) ) || (  in_array( $this->reference->getGatewayStatus() , [ 'PENDING' , 100 ] )  && in_array ( $this->reference->getPaymentType(), [ 'novalnetinvoice', 'novalnetprepayment' , 'novalnetcashpayment' , 'novalnetmultibanco' ])  ) && $balanceAmount > 0 ) , 'amount' => $balanceAmount]);
            }
        }
    ...
    public function displayInstalmentCancelAction()
    {
        if (!empty($this->reference)) {
            ...
            return $this->View()->assign(['success' => (in_array($this->reference->getPaymentType(), ['novalnetinvoiceinstalment', 'novalnetsepainstalment']) && $this->reference->getGatewayStatus() == 'CONFIRMED' && ((int) $this->reference->getAmount() >= (int) $this->reference->getRefundedAmount()) && ($instalmentCancelExecuted == false) )]);
        }
    ...
    public function processInstalmentCancelAction()
    {
        ...
        if ($instalmentcancelResponse['result']['status'] === 'SUCCESS') {
            if (in_array($instalmentcancelResponse['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
                ...
                } else if ($cancelType == 'CANCEL_ALL_CYCLES') {
