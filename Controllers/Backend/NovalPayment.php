...
class Shopware_Controllers_Backend_NovalPayment extends Shopware_Controllers_Backend_ExtJs
{
    ...
    public function processRefundAction()
    {
        ...
        if ($response['result']['status'] === 'SUCCESS') {
            ...
            $refundTID = (in_array($this->reference->getPaymentType(), ['novalnetinvoiceinstalment','novalnetsepainstalment'])) ? $response['transaction']['tid'] : $this->reference->getTid();
            
            $message = sprintf($this->helper->getLanguageFromSnippet('frontend_novalnet_refund'), $refundTID, $amountInBiggerCurrencyUnit, date($this->dateformatter. ' H:i:s'));

            if (!empty($response['transaction']['refund']['tid'])) {
                $message = sprintf($this->helper->getLanguageFromSnippet('frontend_novalnet_refund1'), $refundTID, $amountInBiggerCurrencyUnit, date($this->dateformatter. ' H:i:s'), $response['transaction']['refund']['tid']);
            }
    ...
    public function processInstalmentCancelAction()
    {
        ...
        if ($response['result']['status'] !== 'SUCCESS') {
            $message = $this->helper->getStatusDesc($response, $this->helper->getLanguageFromSnippet('BasicParamError'));
            return $this->View()->assign(['success' => false, 'message' => $message]);
        }
    
        $transactionStatus = $response['transaction']['status'];
        $validStatuses = ['CONFIRMED', 'PENDING'];
    
        if (!in_array($transactionStatus, $validStatuses)) {
            return $this->View()->assign(['success' => false, 'message' => 'Invalid transaction status']);
        }
    
        $configDetails = $this->helper->unserializeData($this->reference->getConfigurationDetails());
        $updatedConfigDetails = $this->helper->updateConfigurationDetails($configDetails, $cancelType, null);
        
        $this->databaseHandler->insertNovalnetTransaction([
            'tid'                   => $response['transaction']['tid'],
            'configuration_details' => $updatedConfigDetails['instalmentConfigData'],
        ...
        if ($cancelType === 'CANCEL_REMAINING_CYCLES') {
            $messageTemplate = 'frontend_novalnet_instalment_cancel_remaining';
        } elseif ($cancelType === 'CANCEL_ALL_CYCLES') {
            $messageTemplate = 'frontend_novalnet_instalment_cancel_all';
    
            if (!empty($response['transaction']['refund']['amount'])) {
                $formattedRefundAmount = $this->helper->amountInBiggerCurrencyUnit($response['transaction']['refund']['amount'], $request['currency']);
                $messageTemplate .= '_with_refund';
            }
        }
    
        $message = sprintf(
            $this->helper->getLanguageFromSnippet($messageTemplate),
            $this->reference->getTid(),
            date($this->dateformatter . ' H:i:s'),
            $formattedRefundAmount
        );
    
        if ((int)$updatedConfigDetails['refundTotalAmount'] >= (int)$this->reference->getAmount()) {
            Shopware()->Modules()->Order()->setPaymentStatus($request['id'], '35', false);
        }
    
        $this->databaseHandler->postProcess($message, ['tid' => $response['transaction']['tid']], true);
    
        return $this->View()->assign(['success' => true, 'message' => $message]);
    }
}
