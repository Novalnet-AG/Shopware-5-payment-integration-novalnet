...
class Shopware_Plugins_Frontend_NovalPayment_lib_classes_NovalnetHelper
{
    ...
    public function prepareComments($paymentShortName, $response, $currency, $testMode, $orderNo, $productId, $lang = 'de', $configuration = array())
    {
        ...
        $formattedAmount = (($lang == 'de') ? str_replace('.', ',', $response['amount']) : $response['amount']) . ' ' . $currency;
        ...
        switch ($paymentShortName) {
            ...
            case 'novalnetprepayment':
                ...
                if (!in_array($response['tid_status'],array('75'))) {
                    ...
                    {
						$note .= sprintf($this->novalnetLang['novalnet_payment_mail_invoice_with_due_date'], $formattedAmount, $response['due_date']) . $newLine. $newLine;
					} else {
						$note .= sprintf($this->novalnetLang['novalnet_payment_mail_invoice'], $formattedAmount) . $newLine. $newLine;
					}
    ...
    public function getCommonRequestParams($configDetails, $nnCustomerData, $paymentShortName)
    {
        ...
        $urlparam = array_merge($nnCustomerData, array(
            ...
            'remote_ip' => $remoteIp,
            'system_version' => Shopware()->Config()->get('Version') . '-NN11.3.5',
    ...
    */
     ...
     * @param $user_details
     * @param $reference_details
     * @param $change_payment
     * @return null
     */
    public static function formParams(&$params, $config_details, $user_details, $reference_details, $change_payment = false)
    {
