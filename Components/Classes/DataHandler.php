...
class DataHandler
{
    ...
    public function postProcess($comment, $data = [], $append = true, $zeroAmountTid = null)
    {
        ...
        if (!empty($append)) {
            ...
            Shopware()->Db()->query('update s_order set customercomment = CONCAT(customercomment,?), transactionID = ? ,temporaryID = ? where transactionID = ?', [$comment,$data['tid'],$data['tid'],$zeroAmountTid]);
        } else {
