...
class NovalnetHelper
{
    ...
    public function formCustomerComments($novalnetResponse, $payment, $currency)
    {
        ...
        if($novalnetResponse ['transaction']['payment_type'] == 'GOOGLEPAY' && !empty($novalnetResponse['transaction']['payment_data']))
        {
    ...
    {
        $country['id'] = Shopware()->Session()->get('sCountry') ? Shopware()->Session()->get('sCountry') : $this->getSelectedCountry();
    ...
    */
    public function updateCustomerData($userData, $response)
    {
        ...
        foreach ($sheetAry as $key => $value) {
            $userData['billingaddress'][$key]  = ($value == 'addressLines') ? $response['order']['billing']['contact'][$value][0].' '. $response['order']['billing']['contact'][$value][1] : $response['order']['billing']['contact'][$value];
            $userData['shippingaddress'][$key] = ($value == 'addressLines') ? $response['order']['shipping']['contact'][$value][0].' '. $response['order']['shipping']['contact'][$value][1] : $response['order']['shipping']['contact'][$value];
        }
