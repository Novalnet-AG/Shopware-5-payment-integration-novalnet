...
define("NOVALNET_PLUGIN_VERSION", "13.0.0");
...
class NovalnetHelper
{
    ...
    {
        $country = ! empty(Shopware()->Session()->get('sCountry')) ? Shopware()->Modules()->Admin()->sGetCountry(Shopware()->Session()->get('sCountry')) : $this->getSelectedCountry();
    ...
    public function getLanguageFromSnippet($snippetName, $ordernumber = null, $namespace = 'frontend/novalnet/payment')
    {
        if (! empty($ordernumber)) {
            $orderShopId = Shopware()->db()->fetchOne('SELECT language from s_order WHERE ordernumber ='. $ordernumber);
    ...
    public function formCustomerComments($novalnetResponse, $currency)
    {
        ...
        $transactionAmount = ! empty($novalnetResponse->getData('instalment/cycle_amount')) ? $novalnetResponse->getData('instalment/cycle_amount') : $novalnetResponse->getData('transaction/amount');
        ...
        if ($nnPaymentType == 'GOOGLEPAY' && !empty($novalnetResponse->getData('transaction/payment_data/card_brand'))) {
            $comments .= sprintf($this->getLanguageFromSnippet('googlePayCardInfo'), strtolower($novalnetResponse->getData('transaction/payment_data/card_brand')), $novalnetResponse->getData('transaction/payment_data/last_four')). $this->newLine;
        }
        ...
        if (!empty($novalnetResponse->getData('transaction/bank_details')) && (! preg_match("/DIRECT_DEBIT_SEPA/i", $nnPaymentType)) &&
            !( $nnGatewayStatus == NOVALNET_PENDING_STATUS &&  (preg_match("/GUARANTEED/i", $nnPaymentType) ||  preg_match("/INSTALMENT/i", $nnPaymentType)) )
        ) {
            if (in_array($nnGatewayStatus, [NOVALNET_CONFIRMED_STATUS, NOVALNET_PENDING_STATUS]) && ! empty($novalnetResponse->getData('transaction/due_date'))) {
                $comments .= sprintf($this->getLanguageFromSnippet('invoicePaymentBankText'), $amountInBiggerCurrencyUnit, date('d.m.Y', strtotime($novalnetResponse->getData('transaction/due_date')))) . $this->newLine. $this->newLine;
            } else {
            ...
            foreach ($nnBankDataText as $key => $text) {
                if (! empty($novalnetResponse->getData('transaction/bank_details')[$key])) {
                    $comments .= $text . $novalnetResponse->getData('transaction/bank_details')[$key] . $this->newLine;
                }
            }
        ...
        if (!empty($novalnetResponse->getData('transaction/nearest_stores'))) {
            if ($novalnetResponse->getData('transaction/due_date')) {
                $comments .= $this->getLanguageFromSnippet('slipExpiryDate') . ': ' . date('d.m.Y', strtotime($novalnetResponse->getData('transaction/due_date'))) . $this->newLine;
            }

            $comments .= $this->newLine . $this->getLanguageFromSnippet('cashPaymentStores') . $this->newLine;

            foreach ($novalnetResponse->getData('transaction/nearest_stores') as $nearestStore) {
    ...
    */
    ...
    public function sendNovalnetOrderMail($variables, $instalment = false)
    {
        ...
        $lang = ! empty(strstr($shopLang, 'en')) ? 'en' : 'de';
    ...
    public function getPaymentFormUrl($enabledPaymentMethods, $changeSupportedPayments = false)
    {
        ...
        if ($isActiveNNPay) {
            ...
            if ($changeSupportedPayments) {
                $paymentFormData['hosted_page']['display_payments'] = ['DIRECT_DEBIT_SEPA', 'INVOICE', 'CASHPAYMENT', 'PREPAYMENT', 'CREDITCARD'];
            }
    ...
    public function handleResponseData($response, $orderNumber = null)
    {
        ...
        if (!empty($response)) {
            ...
            $insertKey = ['transaction/payment_name', 'transaction/test_mode', 'transaction/due_date', 'transaction/payment_data/token', 'transaction/ref_payment_data', 'customer/birth_date'];
            foreach ($insertKey as $key) {
                if (!empty($response->getData($key))) {
                    $splitedArray = explode('/', $key);
                    $configurationKey = end($splitedArray);
                    $insertData['configuration_details'][$configurationKey] = $response->getData($key);
                }
            }
            
            if (!empty(Shopware()->Session()->offsetGet('merchant_details'))) {
                $insertData['configuration_details']['merchant'] = Shopware()->Session()->offsetGet('merchant_details');
            }
