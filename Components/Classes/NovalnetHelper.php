...
class NovalnetHelper
{
     ...
     * @var array
     */
    private $nnRedirectPayments = ['novalnetpaypal', 'novalnetonlinebanktransfer', 'novalnetideal', 'novalnetinstant', 'novalnetgiropay', 'novalnetprzelewy24', 'novalneteps', 'novalnetpostfinancecard', 'novalnetpostfinance', 'novalnetbancontact', 'novalnettrustly', 'novalnetalipay', 'novalnetwechatpay', 'novalnetblik', 'novalnettwint'];
     ...
     * @var array
     */
    private $enforcePayments  = ['novalnetcc', 'novalnetgooglepay'];
     ...
     * @return array
     */
    public function getConfigurations($shopId = null)
    {
        $shop = null;
    ...
    /**
     * Set locale to snippet namespace
     *
     * @param string $type
     *
     * @return string
     */
    public function setLocale($orderNumber)
    {
    ...
    {
        $countryId   = $datas['countryId'] ? $datas['countryId'] : $datas['country_id'];
        $countryCode = Shopware()->Db()->fetchOne('SELECT countryiso FROM s_core_countries WHERE  id = ?', [$countryId]);
        $data = [
            'street'    => $datas['street'],
            'city'      => $datas['city'],
            'zip'       => $datas['zipcode'],
    ...
    public function formCustomerComments($novalnetResponse, $payment, $currency)
    {
        ...
        if ($novalnetResponse ['transaction']['payment_type'] == 'GOOGLEPAY' && !empty($novalnetResponse['transaction']['payment_data']['card_brand'])) {
            $comments .= sprintf($this->getLanguageFromSnippet('googlePayCardInfo'), strtolower($novalnetResponse['transaction']['payment_data']['card_brand']), $novalnetResponse['transaction']['payment_data']['last_four']). $this->newLine;
        } elseif ($novalnetResponse ['transaction']['payment_type'] == 'APPLEPAY' && !empty($novalnetResponse['transaction']['payment_data']['card_brand'])) {
            $comments .= sprintf($this->getLanguageFromSnippet('applePayCardInfo'), strtolower($novalnetResponse['transaction']['payment_data']['card_brand']), $novalnetResponse['transaction']['payment_data']['last_four']). $this->newLine;
        }
        ...
        switch ($payment) {
            ...
            case 'novalnetprepayment':
                if ($novalnetResponse['transaction']['status'] === 'PENDING' && in_array($payment, ['novalnetinvoiceGuarantee', 'novalnetinvoiceinstalment'])) {
                    ...
                    $comments .= $this->getLanguageFromSnippet('referenceText1') . '&nbsp;' . $novalnetResponse['transaction']['tid'];
            ...
            default:
                if ($novalnetResponse['transaction']['status'] === 'PENDING' && in_array($payment, ['novalnetsepaGuarantee', 'novalnetsepainstalment'])) {
                    ...
                    $comments .= $this->getLanguageFromSnippet('multibancoPartnerReference') . $novalnetResponse['transaction']['partner_payment_reference'] . $this->newLine;
                    $comments .= $this->getLanguageFromSnippet('multibancoEntityReference') . $novalnetResponse['transaction']['service_supplier_id'];
                }
                break;
        }
    ...
    public function amountInBiggerCurrencyUnit($amount, $currency = '')
    {
        ...
        if (!empty($currency) && ! in_array($currency, ['EUR','USD'])) {
            $currencyRepository = Shopware()->Models()->getRepository('Shopware\Models\Shop\Currency')->findOneBy(['currency' => $currency]);
            $formattedValue .= ' ' . ($currencyRepository->getSymbol() ?? $currency);
        }
    ...
    {
        $country = Shopware()->Session()->get('sCountry') ? Shopware()->Modules()->Admin()->sGetCountry(Shopware()->Session()->get('sCountry')) : $this->getSelectedCountry();
    ...
    */
    public function getShippingMethods($data)
    {
        ...
        if (!empty($data['shippingInfo'])) {
            Shopware()->Session()->offsetSet('taxFree', false);
            $decodeData = $this->unserializeData($data['shippingInfo']);
            $country    = $this->getSelectedCountry();

            if (!empty($data['paymentName'])) {
                $paymentID  = Shopware()->Db()->fetchOne('SELECT id FROM s_core_paymentmeans WHERE name = ?', array($data['paymentName']));
                Shopware()->Session()->offsetSet('sPaymentID', $paymentID);
            }

            if ($decodeData['countryCode']) {
                $countryID = Shopware()->Db()->fetchOne('SELECT id FROM s_core_countries WHERE countryiso = ?', array($decodeData['countryCode']));
            ...
            Shopware()->Session()->offsetSet('sDispatch', $decodeData['identifier']);
        }
    ...
    public function getCartItems()
    {
        ...
        if (!empty($sBasket)) {
            ...
            if (Shopware()->Session()->offsetGet('sDispatch')) {
                $cartItems[]  = array('label' => $this->getLanguageFromSnippet('frontend_shipping_cost_label'), 'type' => 'SUBTOTAL', 'amount' => round(number_format($shipping['netto'], 2, '.', '') * 100));
            }

            if (empty(Shopware()->Session()->offsetGet('taxFree'))) {
                $amount = !empty($sBasket['AmountWithTaxNumeric']) ? $sBasket['AmountWithTaxNumeric'] : $sBasket['AmountNumeric'];
                $totalAmount = round((number_format($amount, 2, '.', '')) * 100);

                foreach ($sBasket['sTaxRates'] as $key => $taxRate) {
                    $label = !empty(sprintf($this->getLanguageFromSnippet('frontend_tax_label'), round($key) . ' %')) ? sprintf($this->getLanguageFromSnippet('frontend_tax_label'), round($key) . ' %') : 'TAX';
                    $cartItems[]  = array('label' => $label,'type' => 'SUBTOTAL', 'amount' => round(number_format($taxRate, 2, '.', '') * 100));
                }
            } else {
                $totalAmount = round((number_format($sBasket['AmountNetNumeric'], 2, '.', '')) * 100);
            }
    ...
    */
    ...
    {
        $customerData['billing']  = $customerData = $this->getCustomerAddress($response);
        $customerData['shipping'] = $this->getCustomerAddress($response, 'shipping');
        $customerData['password'] = $this->getRandomString();
        $customerData['email'] = $response['order']['billing']['contact']['email'] ? $response['order']['billing']['contact']['email'] : $response['order']['shipping']['contact']['email'];
    ...
    private function registerCustomer($customerData)
    {
        ...
        if (!empty(Shopware()->Session()->get('sPaymentID'))) {
            $paymentID = Shopware()->Session()->get('sPaymentID');
    ...
    private function loginCustomer(Customer $customerModel)
    {
        ...
        Shopware()->Session()->offsetSet('sOneTimeAccount', true);
    ...
    public function getCustomerAddress(array $response, $type = 'billing')
    {
        ...
        return [
            ...
            'phone' => !empty($serverData['phoneNumber']) ? $serverData['phoneNumber'] : $response['order']['shipping']['contact']['phoneNumber']
        ];
    }

    /**
     * Return the countryID and stateID from address details
     *
     * @param array $address
     *
     * @return array
     */
    public function getCountryStateId($address)
    {
        $countryId  = Shopware()->Db()->fetchOne('SELECT id FROM s_core_countries WHERE countryiso = ?', array($address['countryCode']));
        $stateId    = null;
        if ($address['administrativeArea']) {
            $stateId = Shopware()->Db()->fetchOne('SELECT id FROM s_core_countries_states WHERE countryID = ? AND shortcode = ?', array($countryId, $address['administrativeArea']));
        }
    ...
    */
    public function updateCustomerData($userData, $response)
    {
        ...
        foreach ($sheetAry as $key => $value) {
            $userData['billingaddress'][$key]  = $response['order']['billing']['contact'][$value];
            $userData['shippingaddress'][$key] = $response['order']['shipping']['contact'][$value];
        }

        $billingCountryStateId  = $this->getCountryStateId($response['order']['billing']['contact']);
        $shippingCountryStateId = $this->getCountryStateId($response['order']['shipping']['contact']);
        ...
        if (!empty($billingCountryStateId['stateId'])) {
            $userData['billingaddress']['stateId'] = $userData['billingaddress']['stateID'] = $billingCountryStateId['stateId'];
        }

        if (!empty($shippingCountryStateId['stateId'])) {
            $userData['shippingaddress']['stateId'] = $userData['shippingaddress']['stateID'] = $shippingCountryStateId['stateId'];
        }
    ...
    */
    ...
    public function canShowCancelAllInstalments($configurationDetails)
    {
        $cancelAllInstalment = null;
        $getConfig = $this->unserializeData($configurationDetails);
    ...
    public function canShowCancelRemainigInstalments($configurationDetails)
    {
