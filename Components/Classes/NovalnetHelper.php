...
class NovalnetHelper
{
    ...
    public function formCustomerComments($novalnetResponse, $payment, $currency)
    {
        ...
        return str_replace('<br />', PHP_EOL, $note);
    }
        ...
        if (!empty($configDetails['InstalmentDetails'])) {
            ...
            foreach ($configDetails['InstalmentDetails'] as $instalmentKey => $instalmentValue) {
                if (is_array($instalmentValue)) {
                    ...
                    if ( ( $cancelType == 'CANCEL_ALL_CYCLES') || ( $instalmentValue['refundedAmount'] >= $instalmentValue['amount'] ) ) {
                    ...
                    } else if ( $cancelType == 'CANCEL_REMAINING_CYCLES' && empty ($instalmentValue['reference']) ) {
                        $status = $this->getLanguageFromSnippet('canceledMsg');
                        $refundedAmount = 0;
                        $InstalmentRefundedAmount += $instalmentValue['amount'];
                    } else {
