...
class PaymentNotification
{
    ...
    public function startProcess()
    {
        ...
        $this->configDetails    = $this->helper->getConfigurations($this->orderData['shopId']);
    ...
    public function getOrderReference()
    {
        ...
        if (!empty($this->eventData['transaction']['order_no']) && empty($dbVal)) {
            $dbVal = $this->queryHandler->checkTransactionExists(['orderNo' => $this->eventData['transaction']['order_no']]);
        }
        ...
        if (!empty($this->eventData['transaction']['order_no'])) {
            $this->helper->setLocale($this->eventData['transaction']['order_no']);
        }
    ...
    public function communicationFailure()
    {
        ...
        $currency = Shopware()->Db()->fetchRow('SELECT * FROM s_core_currencies WHERE id = ?', array($orderData['currencyID']));
        $currencyRepository = Shopware()->Models()->getRepository('Shopware\Models\Shop\Currency')->findOneBy(['id' => $orderData['currencyID']]);
        Shopware()->Session()->offsetSet('sPaymentID', $orderData['paymentID']);
        Shopware()->Session()->offsetSet('sUserId', $orderData['userID']);
        Shopware()->Session()->offsetSet('sBasketCurrency', $orderData['currencyID']);
        ...
        Shopware()->Shop()->setCurrency($currencyRepository);
        ...
        if (empty($orderData['ordernumber']) && !empty($orderData)) {
            ...
            $langCode = strtolower(substr($this->helper->getLocale(), 0, 2));
            ...
            if ($BasketTotalAmount != $this->eventData['transaction']['amount']) {
                ...
                $template = str_replace('#PAYMENT_METHOD#', $paymentData['description'], $template);
                ...
                $this->setViews->message = 'The basket amount '. $formattedBasketTotalAmount. ' is  differs from the Novalnet Booked Amount (' . $formattedNnBookedAmount . ')for the TID :'. $this->parentTid;
                return;
            }
    ...
    private function updateInstalmentRefundInfo()
    {
        ...
        if (! empty($this->eventData['transaction']['refund']['amount'])) {
            foreach ($configurationDetails['InstalmentDetails'] as $instalmentData) {
                if ($instalmentData['reference'] == $this->parentTid) {
                    $totalRefundedAmount = (int)$instalmentData['refundedAmount'] + $this->eventData['transaction']['refund']['amount'];
                    $statusSnippet = ($totalRefundedAmount >= $instalmentData['amount']) ? 'refundedMsg' : 'paidMsg';
                    $configurationDetails['InstalmentDetails'][$instalmentData['cycleExecuted']] = array_merge($instalmentData, [
                        'status'         => $this->helper->getLanguageFromSnippet($statusSnippet),
                        'refundedAmount' => (int)$totalRefundedAmount
                    ]);
                }
            }
        }
    ...
    private function updateInstalmentInfo()
    {
        ...
        $configurationDetails['InstalmentDetails'][$instalmentData['cycles_executed']] = [
            ...
            'refundedAmount'=> (int)$instalmentData['refundedAmount'],
        ];
        return $this->helper->serializeData($configurationDetails);
    }
    ...
    private function transactionRefund()
    {
        ...
        if (! empty($refundAmount)) {
            ...
            if ($totalRefundedAmount <= $this->orderReference->getAmount()) {
                ...
                if (in_array($this->eventData['transaction']['payment_type'], ['INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                    $upsertData['configuration_details'] = $this->updateInstalmentRefundInfo();
                }
    ...
    private function creditProcess()
    {
        ...
        } elseif (in_array($this->eventData['transaction']['payment_type'], [ 'CREDIT_ENTRY_SEPA', 'DEBT_COLLECTION_SEPA', 'CREDIT_ENTRY_CREDITCARD', 'DEBT_COLLECTION_CREDITCARD', 'CREDITCARD_REPRESENTMENT', 'BANK_TRANSFER_BY_END_CUSTOMER', 'GOOGLEPAY_REPRESENTMENT', 'APPLEPAY_REPRESENTMENT', 'DEBT_COLLECTION_DE', 'CREDIT_ENTRY_DE'])) {
            $callbackComments = $this->newLine . sprintf($this->helper->getLanguageFromSnippet('novalnet_callback_credit_message'), $this->parentTid, $this->formattedAmount, date('d-m-Y H:i:s'), $this->eventTid);
    ...
    {
        $cycleAmount = $this->helper->amountInBiggerCurrencyUnit($this->eventData['instalment']['cycle_amount'], $this->eventData['transaction']['currency']);
        $comments = sprintf($this->helper->getLanguageFromSnippet('novalnet_callback_instalment_prepaid_message'), $this->parentTid, $this->eventTid, $cycleAmount, date('d/m/Y H:i:s')).$this->newLine;
        ...
        if (in_array($this->eventData['transaction']['payment_type'], ['INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA']) && $this->orderReference->getGatewayStatus() == 'CONFIRMED') {
            if (! empty($this->eventData['instalment']['prepaid'])) {
                $comments .= $this->helper->formInstalmentComments($this->eventData, $cycleAmount);
            } else {
    ...
    {
        $gatewayStatus = $this->orderReference->getGatewayStatus();
        $validStatuses = ['CONFIRMED', 'PENDING', '100', '75'];

        if (in_array($gatewayStatus, $validStatuses)) {
            $cancel_type = $this->eventData['instalment']['cancel_type'] ?? 'ALL_CYCLES';
            ...
            $refundAmount = $this->eventData['transaction']['refund']['amount'] ?? null;
            if (!empty($refundAmount)) {
                $callbackComments = $this->newLine . sprintf(
                    $this->helper->getLanguageFromSnippet('novalnet_callback_refund_message'),
                    $this->parentTid,
                    $this->formattedAmountRefund
                );

                $refundTid = $this->eventData['transaction']['refund']['tid'] ?? null;
                if (!empty($refundTid)) {
                    $callbackComments .= sprintf(
                        $this->helper->getLanguageFromSnippet('novalnet_callback_refund_new_tid_message'),
                        $refundTid
                    );
                }

                if ((int)$configurationDetails['refundTotalAmount'] >= (int)$this->orderReference->getAmount()) {
                    Shopware()->Modules()->Order()->setPaymentStatus((int)$this->orderData['id'], 35, false);
                }
            }

            $callbackTemplate = ($cancel_type == 'REMAINING_CYCLES')
                ? 'frontend_novalnet_instalment_cancel_remaining'
                : 'frontend_novalnet_instalment_cancel_all';

            $callbackComments = sprintf(
                $this->helper->getLanguageFromSnippet($callbackTemplate),
                $this->orderReference->getTid(),
                date('d/m/Y H:i:s')
            );
            ...
            if (!empty($refundAmount)) {
                $callbackTemplate = ($cancel_type == 'REMAINING_CYCLES')
                    ? 'frontend_novalnet_instalment_cancel_remaining'
                    : 'frontend_novalnet_instalment_cancel_all_with_refund';
            ...
            $this->queryHandler->postProcess($callbackComments, $upsertData, true);
        }
     ...
     * @return void
     */
    public function sendNotifyMail($mailSubject = null, $mailBody = null)
    {
