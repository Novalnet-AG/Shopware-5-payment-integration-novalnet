...
class PaymentNotification
{
    ...
    private function startProcess()
    {
        ...
        if ('SUCCESS' == $this->eventData['result']['status'] && 'FAILURE' != $this->eventData['transaction']['status'] && (!empty($this->orderReference) || $this->eventType == 'PAYMENT')) {
            ...
            $this->setViews->message = ! empty($this->eventData['result']['status'] != 'SUCCESS') ? 'Novalnet callback received. Status is not valid.' : 'Novalnet callback received. Callback Script executed already.';
        }
    ...
    private function checkIp()
    {
        ...
        if (!empty($requestReceivedIp) && !empty($novalnetHostIp)) {
            if ($novalnetHostIp !== $requestReceivedIp && empty($this->configDetails['novalnetcallback_test_mode'])) {
    ...
    private function sendNotifyMail($mailSubject = null, $mailBody = null)
    {
        ...
        $lang     = ! empty($this->helper->getLocale()) ? $this->helper->getLocale() : 'de_DE';
    ...
    {
        return ! empty($this->orderReference->getGatewayStatus()) ? $this->helper->getStatus($this->orderReference->getGatewayStatus(), $this->orderReference, $this->eventData['transaction']['payment_type']) : null ;
    }
}
