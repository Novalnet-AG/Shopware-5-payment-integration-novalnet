...
class PaymentNotification
{
    ...
    {
        if (in_array($this->orderReference->getGatewayStatus(), ['CONFIRMED', 'PENDING', '100' , '75'])) {
            ...
            if (! empty($this->eventData['transaction']['refund']['amount'])) {
                ...
                $callbackComments = ( $cancel_type == 'REMAINING_CYCLES' ) ? sprintf($this->helper->getLanguageFromSnippet('frontend_novalnet_instalment_cancel_remaining'), $this->orderReference->getTid(), date('d/m/Y H:i:s')) : sprintf($this->helper->getLanguageFromSnippet('frontend_novalnet_instalment_cancel_all'), $this->orderReference->getTid(), date('d/m/Y H:i:s'), $this->formattedAmountRefund) ;
