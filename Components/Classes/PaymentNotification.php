...
class PaymentNotification
{
    ...
    public function communicationFailure()
    {
        ...
        if (empty($orderData['ordernumber']) && !empty($orderData)) {
            ...
            $formattedNnBookedAmount = $this->helper->setHtmlEntity($this->helper->amountInBiggerCurrencyUnit($NnBookedAmount, $this->eventData['transaction']['currency']), $type = 'encode');
    ...
    private function instalmentCancelProcess()
    {
        if (in_array($this->orderReference->getGatewayStatus(), ['CONFIRMED', 'PENDING', '100' , '75'])) {
            ...
            if (!empty( $this->event_data['instalment']['cancel_type'] ) ) {
                $cancel_type = $this->event_data['instalment']['cancel_type'];
            }
            $configurationDetails = $this->helper->unserializeData($this->orderReference->getConfigurationDetails());
