...
class NovalnetValidator
{
     ...
     * @var NovalnetHelper
     */
    private $aboSupportedPayments = ['novalnetsepa', 'novalnetinvoice', 'novalnetcashpayment', 'novalnetprepayment', 'novalnetcc', 'novalnetpaypal', 'novalnetinvoiceGuarantee', 'novalnetsepaGuarantee'];
    ...
    public function displayValidPayments($enabledPaymentMethods)
    {
        ...
        foreach ($enabledPaymentMethods as $key => $payment) {
            if (strpos($payment['name'], 'novalnet') !== false) {
                if (!$this->config['novalnet_secret_key'] || !$this->config['novalnet_tariff']) {
                    ...
                    if (in_array($payment['name'], ['novalnetsepaGuarantee', 'novalnetsepa']) && $sepaGuaranteeEnabled) {
                        ...
                        if (!$guaranteeCheck && $payment['name'] == 'novalnetsepaGuarantee') {
                            ...
                            $sepaPayment = $enabledPaymentMethods[$key];
                    ...
                    if (in_array($payment['name'], ['novalnetinvoiceGuarantee', 'novalnetinvoice']) && $invoiceGuaranteeEnabled) {
                        ...
                        if (!$guaranteeCheck && $payment['name'] == 'novalnetinvoiceGuarantee') {
                            ...
                            $invoicePayment = $enabledPaymentMethods[$key];
                ...
                } elseif ((!stripos($userAgent, "Mac OS") || !stripos($userAgent, "Safari") || preg_match($pattern, $userAgent, $matchedAgent)) && $payment['name'] == 'novalnetapplepay') {
                    unset($enabledPaymentMethods[$key]);
                }
                ...
                foreach ($basketContent as $content) {
                    if (!empty($content['__s_order_basket_attributes_swag_abo_commerce_id'])) {
                        $totalAmount = $content['amountNumeric'];
                        if (!in_array($payment['name'], $this->aboSupportedPayments)) {
                            unset($enabledPaymentMethods[$key]);
                        } elseif (in_array($payment['name'], array('novalnetinvoiceGuarantee', 'novalnetsepaGuarantee')) && $totalAmount < 9.99) {
                            unset($enabledPaymentMethods[$key]);
                            if (!empty($this->config['novalnetinvoiceGuarantee_force_payment']) && $payment['name'] == 'novalnetinvoiceGuarantee') {
                                array_push($enabledPaymentMethods, $invoicePayment);
                            } elseif (!empty($this->config['novalnetsepaGuarantee_force_payment']) && $payment['name'] == 'novalnetsepaGuarantee') {
                                array_push($enabledPaymentMethods, $sepaPayment);
                            }
                        }
                    }
                }
    ...
    public function checkGuarantee(string $payment)
    {
        ...
        $sBasketAmount  = $this->helper->getAmount();
    ...
    /**
     * To validate company and control Date of Birth field visibility
     *
     * @param string $paymentName
     * @return bool
    */
    public function canShowDobField($paymentName)
    {
        $userData = Shopware()->Session()->get('sOrderVariables')['sUserData'] ? Shopware()->Session()->get('sOrderVariables')['sUserData'] : $this->helper->getUserInfo();
        if (!empty($userData['billingaddress']['company']) && $this->config[$paymentName.'_allow_b2b'] != false) {
            if (preg_match('/^(?:\d+|(?:)\.?|[^a-zA-Z0-9]+|[a-zA-Z]{1})$|^(herr|frau|jahr|mr|miss|mrs|others|andere|anrede|salutation|null|none|keine|company|firma|no|na|n\/a|test|private|privat)$/i', $userData['billingaddress']['company'])) {
                return 'true';
            } else {
                return 'false';
            }
