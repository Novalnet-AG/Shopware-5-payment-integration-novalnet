...
class PaymentRequest
{
    ...
    public function getRequestParams($getPaymentFrom = false)
    {
        ...
        if (Shopware()->Container()->has('session')) {
            if (!empty(Shopware()->Session()->offsetGet('novalnetPay'))) {
            ...
            if (Shopware()->Session()['sOrderVariables']['sUserData'] || $this->helper->getUserInfo()) {
                $userDetails = ! empty(Shopware()->Session()['sOrderVariables']['sUserData']) ? Shopware()->Session()['sOrderVariables']['sUserData'] : $this->helper->getUserInfo();
            }
        }
        ...
        if ($getPaymentFrom) {
            ...
            $themeName = Shopware()->Container()->get('models')->getRepository(Template::class)->findOneBy(['id' => $themeId])->getTemplate();
    ...
    public function getTransactionDetails($sessionData = [])
    {
        $transactionData = [
            ...
            'system_version' => Shopware()->Config()->get('Version') . '-NN13.0.0',
        ...
        if (!empty($sessionData)) {
            ...
            $paymentDataKeys = ['booking_details/pan_hash', 'booking_details/unique_id', 'booking_details/iban', 'booking_details/bic', 'booking_details/wallet_token', 'booking_details/payment_ref/token'];
    ...
    public function gethostedPageDetails()
    {
        ...
        if ($isAboProduct) {
            $hostedPageData['display_payments'] = ['DIRECT_DEBIT_SEPA', 'INVOICE', 'CASHPAYMENT', 'PREPAYMENT', 'CREDITCARD', 'PAYPAL', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA'];
            $totalAmount  = $this->helper->getAmount();
            if ($totalAmount < 9.99) {
                $hostedPageData['display_payments'] = ['DIRECT_DEBIT_SEPA', 'INVOICE', 'CASHPAYMENT', 'PREPAYMENT', 'CREDITCARD', 'PAYPAL'];
            }
        }
    ...
    public function getCartInfo(&$parameters)
    {
        ...
        if (!empty(Shopware()->Session()->get('sOrderVariables')['sBasket'])) {
            foreach (Shopware()->Session()->get('sOrderVariables')['sBasket']['content'] as $content) {
        ...
        if (empty(Shopware()->Session()->offsetGet('taxFree'))) {
            foreach (Shopware()->Session()->get('sOrderVariables')['sBasket']['sTaxRates'] as $key => $taxRate) {
        ...
        if (!empty($sBasket)) {
            ...
            if (empty(Shopware()->Session()->offsetGet('taxFree'))) {
                foreach ($sBasket['sTaxRates'] as $key => $taxRate) {
                    $label = !empty(sprintf($this->helper->getLanguageFromSnippet('frontend_tax_label'), round($key) . ' %')) ? sprintf($this->helper->getLanguageFromSnippet('frontend_tax_label'), round($key) . ' %') : 'TAX';
