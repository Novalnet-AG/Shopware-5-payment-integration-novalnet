...
class PaymentRequest
{
    ...
    public function getTransactionDetails($router, $uniquePaymentID, $currentPayment, $isRecurring = false)
    {
        ...
        $transactionData = [
            ...
            'system_version' => Shopware()->Config()->get('Version') . '-NN12.3.2',
    ...
    public function paypalSheetDetails(&$parameters)
    {
		...
		if (!empty($this->orderDetails['sBasket']))
		{
			...
			{   
				...
				$parameters['cart_info']['line_items'][] = array( 'name'=> $content['articlename'], 'price' => round((float) sprintf('%0.2f', $content['netprice']) * 100), 'quantity' => $content['quantity'], 'description' => $content['additional_details']['description_long'], 'category' => $productType );
			}
		}
