...
class PaymentRequest
{
    ...
    public function getTransactionDetails($router, $uniquePaymentID, $currentPayment, $isRecurring = false)
    {
        ...
        $transactionData = [
            ...
            'system_version' => Shopware()->Config()->get('Version') . '-NN12.3.0',
    ...
    public function getApplePayParams($requiredFields = false, $product = array())
    {
        ...
        $data['transaction'] = [
            ...
            'environment' => empty($this->configDetails['novalnetapplepay_test_mode']) ? 'LIVE' : 'SANDBOX' 
        ];
    ...
    {	
        ...
        if (empty($articleData['displayItems']))
		{
			$label = $product['articleName']. ' (1 x '. html_entity_decode(Shopware()->Shop()->getCurrency()->getSymbol()). sprintf('%0.2f', $value['price_numeric']).')';
            $cartItems[] = array('label' => $label, 'type' => 'SUBTOTAL', 'amount' => round(number_format($value['price_numeric'], 2, '.', '') * 100));
            $articleData['displayItems'] = $cartItems;
		}
        ...
        $data['transaction'] = [
            ...
            'environment' => empty($this->configDetails['novalnetgooglepay_test_mode']) ? 'LIVE' : 'SANDBOX',
        ...
        $data['button'] = [
			...
			'style'  => $this->configDetails['novalnetgooglepay_button_theme'],
			'locale' => str_replace('_', '-', $this->helper->getLocale()),
			'boxSizing' => 'fill',
