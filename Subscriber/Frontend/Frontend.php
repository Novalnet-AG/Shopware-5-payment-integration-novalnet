...
class Frontend implements SubscriberInterface
{
    ...
    public function onBeforeRenderDocument(\Enlight_Hook_HookArgs $args)
    {
        ...
        if (preg_match("/novalnet/i", $document->_order->payment['name']) != false) {
            ...
            $configData = !empty($getTransaction) ? $getTransaction->getConfigurationDetails() : null;
            $novalnetPaymentDesc = !empty($configData) ? $this->helper->unserializeData($configData)['payment_name'] : null;
            $orderAttrNovalnetDesc = $document->_order->order->attributes['novalnet_payment_name'];
            $orderData['_payment']['description'] = !empty($novalnetPaymentDesc) ? $novalnetPaymentDesc : (!empty($orderAttrNovalnetDesc) ? $orderAttrNovalnetDesc : $orderData['_payment']['description']);
            ...
            if (!empty($orderData['_order']['transactionID'])) {
                $novalnetPayment = '';
                $getTransaction = $this->dataHandler->checkTransactionExists(['tid' => $orderData['_order']['transactionID']]);
                $nnPaymentType = $getTransaction->getPaymentType();

                if (!empty($getTransaction) && !empty($nnPaymentType)) {
                    $novalnetPayment = $nnPaymentType;
                    if (!empty($novalnetPayment)) {
                        $novalnetPayment = $this->helper->getPaymentType($novalnetPayment);
                    }
                }

                if (!empty($novalnetPayment) && in_array($novalnetPayment, ['INVOICE', 'PREPAYMENT', 'GUARANTEED_INVOICE'])) {
                    //update order number for transaction
