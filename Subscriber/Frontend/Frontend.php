...
class Frontend implements SubscriberInterface
{
    ...
    public function onUpdateAbocommercePayment(\Enlight_Hook_HookArgs $args)
    {
        ...
        if ($request->getControllerName() == 'AboCommerce' && $request->getActionName() =='updateAboPayment' && $AboOrderData['selectedPaymentId'] == $AboOrderData['novalnetpayId']) {
            if (in_array(!empty($formResponseData['payment_details']['type']) && $formResponseData['payment_details']['type'], ['CREDITCARD', 'DIRECT_DEBIT_SEPA'])) {
            ...
            if ($formResponseData['result']['status'] == 'SUCCESS' && $formResponseData['result']['statusCode'] == 100) {  //Tingering
                    $changePaymentData = [
