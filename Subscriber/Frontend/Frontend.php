...
class Frontend implements SubscriberInterface
{
    ...
    public function onOrderAddNovalnetCommet(\Enlight_Event_EventArgs $arguments)
    {
        ...
        if (($novalnetResponse['transaction']['status'] != 'PENDING' && !empty($payment) && in_array($payment->getName(), ['novalnetinvoiceinstalment', 'novalnetinvoiceGuarantee'])) || (in_array($novalnetResponse['transaction']['payment_type'], ['INVOICE', 'PREPAYMENT']))) {
    ...
    public function onPreDispatch(\Enlight_Event_EventArgs $args)
    {
        ...
        if ($request->getModuleName() == 'frontend') {
            ...
            $view->isSepaInstalmentActive = $view->isInvoiceInstalmentActive = $view->isApplePayActive = false;

            foreach (Shopware()->System()->sMODULES['sAdmin']->sGetPaymentMeans() as $paymentMeans) {
                if ($paymentMeans['name'] == 'novalnetinvoiceinstalment') {
                    $view->isInvoiceInstalmentActive = true;
                }
                if ($paymentMeans['name'] == 'novalnetsepainstalment') {
                    $view->isSepaInstalmentActive = true;
                }
                if ($paymentMeans['name'] == 'novalnetapplepay' && (!empty(Shopware()->Modules()->Basket()->sGetBasket()) || $request->getControllerName() == 'detail') && in_array($request->getActionName(), $config['novalnetapplepay_button_display_fields']) && !empty($config['novalnet_tariff'])) {
                    $view->isApplePayValid = true;
                }
            }
            ...
            $view->applePayParams = $services->getApplePayParams();
            if ($request->getControllerName() == 'checkout' || $request->getControllerName() == 'account') {
                ...
                if (in_array($currentPayment, array('novalnetcc','novalnetsepa','novalnetsepaGuarantee','novalnetpaypal','novalnetsepainstalment'))) {
    ...
    public function onPostDispatchFrontend(\Enlight_Event_EventArgs $args)
    {
        ...
        if (in_array($request->getControllerName(), ['account','checkout']) && in_array($request->getActionName(), ['payment','confirm','shippingPayment', 'savePayment', 'saveShippingPayment'])) {
    ...
    public function onPostDispatchSecureFrontend(\Enlight_Event_EventArgs $args)
    {
        ...
        if ((in_array($request->getActionName(), array('saveShippingPayment','savePayment', 'payment')) && in_array($paymentName, ['novalnetsepa','novalnetcc','novalnetsepaGuarantee', 'novalnetinvoiceGuarantee', 'novalnetpaypal', 'novalnetinvoiceinstalment', 'novalnetsepainstalment']) && !empty($data[$paymentName. 'FormData'])) || ($request->getControllerName() === 'checkout' && $request->getActionName() === 'payment' && $paymentName == 'novalnetapplepay')) {
